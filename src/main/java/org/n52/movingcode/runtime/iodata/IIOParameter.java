...
public interface IIOParameter extends List{
	 ...
	 *
	 */
	public static enum Direction {IN, OUT, BOTH, NONE;}
	...
	/**
	 * Minimum multiplicity of this parameter:
	 * 0 (zero) means optional
	 * >0 is mandatory
	 * 
	...
	/**
	 * Maximum multiplicity of this parameter:
	 * equal to minMultiplicity means an exactly n-times
	...
	/**
	 * Class for harmonized use of a parameterID.
	 * Stores type and value and provides access and
	 * conversion methods.
	 * 
	 * @author Matthias Mueller, TU Dresden
	 *
	...
	public final class ParameterID implements Comparable{
		public static enum IDType {SEQUENTIAL, NAMED}
		
		private final String stringValue;
		private final int seqVal;
		
		/**
		 * Creates a ParameterID from a BigInteger.
		 * This is usually required with PackageDescription XML.
		 * 
		 * @param position
		 */
		public ParameterID (BigInteger position){
			if (position.compareTo(new BigInteger(Integer.toString(1)))==-1
				...
				throw new IllegalArgumentException("Sequential Values must be greater than zero and smaller than " + (Integer.MAX_VALUE-1) + "!");
			}
		...
		/**
		 * Converts a harmonized value to an INT.
